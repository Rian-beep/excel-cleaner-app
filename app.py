...
if os.path.exists("COMPANY CLEAN LIST.csv"):
    try:
        company_df = pd.read_csv("COMPANY CLEAN LIST.csv")
        known_companies = set(company_df['Cleaned Company'].dropna().str.strip().str.lower())
    except Exception as e:
        st.error(f"Error loading known companies: {e}")
...
def clean_company(name):
    ...
    # Log unknown company name
    with open("unknown_companies_log.csv", "a", encoding="utf-8") as log:
        log.write(original_name.strip() + "\n")
...
def clean_name(name, is_first=True):
    ...
    if not is_first:
        cleaned = format_mc_name(cleaned)
...
def infer_from_email(first, last, email):
    ...
    if len(last) == 1:
        ...
        if match:
            ...
            return first, format_mc_name(last_name)
